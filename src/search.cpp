...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const auto  nmcv  = w.nonMajorCorrectionHistory[non_major_index(pos)][us];
    const auto  cntcv =
      m.is_ok() ? (*(ss - 2)->continuationCorrectionHistory)[pos.piece_on(m.to_sq())][m.to_sq()]
                 : 8;

    return 9536 * pcv + 8494 * micv + 10132 * (wnpcv + bnpcv) + 7156 * cntcv + nmcv * 8000;
}
...
void update_correction_history(const Position& pos,
    ...
    workerThread.nonPawnCorrectionHistory[non_pawn_index<BLACK>(pos)][BLACK][us]
      << bonus * nonPawnWeight / 128;
    workerThread.nonMajorCorrectionHistory[non_major_index(pos)][us] << bonus;
...
void Search::Worker::clear() {
    ...
    nonMajorCorrectionHistory.fill(0);
