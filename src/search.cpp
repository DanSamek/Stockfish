...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).from_to()] << (bonus * 935 / 1024) + 46;
...
moves_loop:  // When in check, search starts here
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).from_to()] << (scaledBonus * 224 / 32768) + 8;

        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            pawnHistory[pawn_structure_index(pos)][pos.piece_on(prevSq)][prevSq]
              << (scaledBonus * 1127 / 32768) + 58;
    }
...
void update_all_stats(const Position& pos,
    ...
    else
    {
        ...
        captureHistory[movedPiece][bestMove.to_sq()][capturedPiece] << (bonus * 1235 / 1024) + 9;
    }
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece] << (-malus * 1354 / 1024) - 29;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    static constexpr int cph[6] = {134, 107, 115, 106, 77, -22};
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    static constexpr int lph[LOW_PLY_HISTORY_SIZE] = {16, -88, -39, 29, -60};
