...
constexpr int RAZORING_HISTORY_BASE_SIZE = 32768;
...
static_assert((RAZORING_HISTORY_BASE_SIZE & (RAZORING_HISTORY_BASE_SIZE - 1)) == 0,
              "RAZORING_HISTORY_BASE_SIZE has to be a power of 2");
...
inline int razoring_history_index(const Position& pos) {
    return pos.pawn_key() & (RAZORING_HISTORY_BASE_SIZE - 1);
}
...
using RazoringHistory = Stats<std::int16_t, 8192, RAZORING_HISTORY_BASE_SIZE, COLOR_NB>;
